...
from ClassInfoHolder import ClassInfoHolder
#to call daikon with
import subprocess
import collections
...
def main():
   ...
   for key in cih_dict:
     name = key.split(":")[0]
     filename = name + "-" + str(args.bucketsize) + "-bucket.dtrace"
     f = open(filename, 'w')
   ...
   with open(filename) as f:
      ...
      for record in records:
         ...
         if (match_declPpt != None):
           ...
           obj_ppt_name_to_cih[split_record[0][4:]] = ClassInfoHolder(split_record, bucketsize)
         # if we are in the dtrace part
            ...
            if split_record[0] in ppt_name_to_parent:
               ...
               cur_cih.add_program_point(split_record)
